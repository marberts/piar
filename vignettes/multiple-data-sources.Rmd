...
scanner_prices <- data.frame(
  period = rep(c("201904", time(elementarys)), each = 200),
  product = 1:200,
...
These values can now be merged with the other elementary indexes, getting
turned into a period-over-period index in the process, and then
aggregated.

```{r}
merge(elementarys, geks_elementarys) |>

...
```{r}
v1 <- transmute_weights2(rel, s1, s2)

all.equal(nested_gmean(rel, s1, s2), gmean(rel, v1))

all.equal(
  v1,
  transmute_weights(rel, transmute_weights2(rel, s1, s2, to = 2), order = 2)
)
```
...
```{r}
v2 <- transmute_weights2(rel, s1, s2, pivot = 1)

all.equal(nested_gmean(rel, s1, s2), gmean(rel, v2))

all.equal(
  v2,
  transmute_weights(rel, transmute_weights2(rel, s1, s2, to = 2, pivot = 1), order = 2)
)
```
...
v <- transmute_weights2(rel, s1, s2, to = -1)

all.equal(
  gmean(V / rel, v),
  nested_gmean(q2 / q1, s1, s2)
)

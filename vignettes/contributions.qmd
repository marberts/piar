...
After an index has been calculated, it's often useful to compute the contribution of higher-level indexes towards the total index. The easiest way to do this with a collection of pre-computed index values is to simply coerce them into a index object with the index values as contributions and reaggregate with a restricted aggregation structure.

...
#' @return A matrix of variances with a row for each upper-level index and a
#' column for each time period.
#' @seealso The `sps_repweights()` function in the \pkg{sps} package to
#' generate replicates weights when elemental aggregates are sampled using
#' sequential Poisson sampling.
#' @references Selvanathan, E. A., and Rao, D. S. P. (1994). *Index
#' Numbers: A Stochastic Approach*. MacMillan.
#' @source The `vcov()` method was influenced by a SAS routine by Justin

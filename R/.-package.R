...
#' @return \code{weights()} returns a list with a named vector of weights for
#' each level in the aggregation structure. If \code{ea_only = TRUE} then the
...
#' 
#' \code{levels()} returns a character vector with the levels for the
#' aggregation structure.
#' @examples
#' 
...
#' \code{\link[=aggregation_structure]{aggregation_structure()}}.
#' @param stringsAsFactors See \code{\link{as.data.frame}}.
#' @param ... Further arguments passed to or used by methods.
#' @return \code{as.matrix()} represents an aggregation structure as a matrix,
...
#' @seealso \code{\link{as_aggregation_structure}} for coercing into an
#' aggregation structure.
#' @examples
#' 
#' # A simple aggregation structure
...
#' \code{\link[=elemental_index]{elemental_index()}}.
#' @param stringsAsFactors See \code{\link{as.data.frame}}.
#' @param ... Further arguments passed to or used by methods.
#' @return \code{as.data.frame()} returns a data frame.
#' 
#' \code{as.matrix()} returns a matrix.
#' @seealso \code{\link{as_index}} to coerce a matrix/data frame of index
...
#' @param ... Ignored.
#' @return A price index that inherits from \code{\link{chainable_piar_index}}
#' if \code{x} is a period-over-period index, or
#' \code{\link{direct_piar_index}} if \code{x} is a fixed-base index. If
#' \code{x} inherits from \code{\link{aggregate_piar_index}} then \code{`[`}
...
#' \code{\link[=aggregation_structure]{aggregation_structure()}}. The default
#' imputes from elemental indexes only (i.e., not recursively).
#' @param w A numeric vector of weights for the prices in \code{x} (i.e.,
...
#' \code{\link[=generalized_mean]{generalized_mean()}} for details.
...
#' or -1 for a harmonic index. Other values are possible; see
#' \code{\link[=generalized_mean]{generalized_mean()}} for details.
#' @return A copy of \code{x} with missing values replaced (where possible).
#' @seealso \code{\link{price_relative}} for making price relatives for the
...
#' giving the replacement levels/time periods for \code{x}.
#' @param ... Further arguments passed to or used by methods.
#' @return \code{levels()} and \code{time()} return a character vector with the
#' levels and time periods for a price index. \code{start()} and \code{end()}
#' return the first and last time period.
#' 
#' The replacement methods return a copy of \code{x} with the levels/time
#' periods in \code{value}. It's not generally possible to change the levels of
...
#' period-over-period \emph{elemental indexes} are first calculated for a
#' collection of \emph{elemental aggregates} at each point in time, and then
#' aggregated according to a \emph{price index aggregation structure}. These
...
#' @docType package
#' @section Usage: The vignette \emph{Making price indexes} gives several
#' extended examples of how to use the functions in this package to make
#' different types of price indexes. Run \code{vignette("making_price_indexes",
...
#' with the \code{\link[=elemental_index]{elemental_index()}} function and an
#' aggregation structure with the
#' \code{\link[=aggregation_structure]{aggregation_structure()}} function. The
#' \code{\link[=aggregate.piar_index]{aggregate()}} method can then be used to
...
#' @author \strong{Maintainer}: Steve Martin \email{stevemartin041@@gmail.com}
#' @seealso \url{https://github.com/marberts/piar}
#' @references Balk, B. M. (2008). \emph{Price and Quantity Index Numbers}.
#' Cambridge University Press.
#' 
#' Chiru, R., Huang, N., Lequain, M. Smith, P., and Wright, A. (2015).
#' \emph{The Canadian Consumer Price Index Reference Paper}, Statistics Canada
#' catalogue 62-553-X. Statistics Canada.
#' 
#' ILO, IMF, OECD, Eurostat, UN, and World Bank. (2020). \emph{Consumer Price
#' Index Manual: Theory and Practice}. International Monetary Fund.
#' 
#' von der Lippe, P. (2001). \emph{Chain Indices: A Study in Price Index
